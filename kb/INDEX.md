# Knowledge Index

> **Last updated**: 2026-02-06

Living summary of all accumulated knowledge. **Read this at session start** to know what you know without opening every file.

## Current Challenge

_Not assigned yet._

## Research

### Hypotheses
_None yet._

<!-- Format when populated:
| ID | Statement | Status |
|---|---|---|
| H001 | Short description | PROPOSED / TESTING / CONFIRMED / REJECTED |
-->

### Experiments
_None yet._

<!-- Format when populated:
| ID | Tests | Status | Key result |
|---|---|---|---|
| E001 | H001 | COMPLETED | Brief outcome |
-->

### Findings
_None yet._

<!-- Format when populated:
| ID | Finding | Impact |
|---|---|---|
| F001 | Short description | HIGH / MEDIUM / LOW |
-->

### Literature
_None yet._

<!-- Format when populated:
| ID | Topic | Relevance |
|---|---|---|
| L001 | Short description | HIGH / MEDIUM / LOW |
-->

## Engineering

### Features
_None yet._

<!-- Format when populated:
| ID | Feature | Status |
|---|---|---|
| FT001 | Short description | PROPOSED / INVESTIGATING / IN PROGRESS / DELIVERED |
-->

### Investigations
_None yet._

### Implementations
_None yet._

## Key Decisions

| # | Type | Decision |
|---|---|---|
| D001 | ENG | Dual-flow project structure (research vs engineering) |

## Lessons Learned

_See also MEMORY.md for cross-session operational patterns._

---

**Maintenance rule**: Update this file every time you create, close, or significantly update any kb/ artifact. Keep each entry to one line. Details live in the individual files.
